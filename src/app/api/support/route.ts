...
export async function POST(req: NextRequest) {
  try {
    ...
    await resend.emails.send({
      ...
      replyTo: senderEmail,
      subject: `[CMB ${type || 'Support'}] ${subject}`,
      html: `
        <div style="font-family: Georgia, serif; max-width: 600px; margin: 0 auto; background: #2D1B69; color: #D4B896; padding: 24px; border-radius: 12px;">
          <h2 style="color: #C9A84C; margin-bottom: 8px;">🏛️ Connect My Brother — Support Request</h2>
          <hr style="border-color: #C9A84C; opacity: 0.3; margin-bottom: 16px;" />
          <p><strong style="color: #C9A84C;">From:</strong> ${senderName} (${senderEmail})</p>
          <p><strong style="color: #C9A84C;">Type:</strong> ${type || 'General Support'}</p>
